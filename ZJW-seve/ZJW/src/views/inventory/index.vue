<template>
  <div class="table-container">
    ...
    <el-table :data="expressData.slice((currentPage-1)*pageSize,currentPage*pageSize)" class="table">
      ...
      >
        <template v-slot="{ row }">
          <el-button v-if="row.is_send&&row.is_receive===false" type="warning" @click="cancelSendExpress(row.item_id)">取消揽件</el-button>
          <el-button v-else-if="row.is_send===false&&row.is_receive===false" type="primary" @click="AsendExpress(row.item_id)">揽件</el-button>
          <el-button v-else-if="row.is_receive===true" type="danger" :disabled="true">已收件</el-button>
        </template>
      </el-table-column>
      <el-table-column
        fixed="right"
      ...
      >
        <template v-slot="{ row }">
          <el-button v-if="row.is_send&&row.is_receive==true" type="warning" @click="cancelExpress(row.item_id)">取消收件</el-button>
          <el-button v-else-if="row.is_send&&row.is_receive===false" type="primary" @click="collectExpress(row.item_id)">收件</el-button>
          <el-button v-else-if="row.is_send===false" type="danger" :disabled="true">未揽件</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods:
  {
    async collectExpress(item) {
      await receiveExpress({item_id:item})
      this.getExpressCondition()
      this.$message.success('收件成功')
    },
    async cancelExpress(item) {
      await cancelReceiveExpress({item_id:item})
      this.getExpressCondition()
      this.$message.success('取消收件成功')
    },
    async AsendExpress(item) {
      await sendExpress({item_id:item})
      this.getExpressCondition()
      this.$message.success('揽件成功')
    },
    async cancelSendExpress(item) {
      await cancelSendExpress({item_id:item})
      this.getExpressCondition()
